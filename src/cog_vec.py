...
def _det_seed(*parts) -> int:
    """确定性整数种子（★v3.4 修复 PYTHONHASHSEED 暗雷）。

    严禁用内置 hash() 为 str 播种：str.__hash__ 受 PYTHONHASHSEED 影响，
    同一份代码在不同进程里会得到不同的随机流，污染"跨进程保持"与
    "跑两次分数相同"两条铁律。md5 摘要跨进程/跨平台恒定。

    :param parts: 任意可 str 化的成分，按给定顺序拼接
    :return: [0, 2**32) 内的确定性整数
    """
    h = hashlib.md5(("\x1f".join(str(p) for p in parts)).encode("utf-8")).digest()
...
class NervousSystem:
    ...
    def _infer_tract_tier(self, member_ids: List[int]) -> str:
        ...
        """
        # ★v3.4：改用 sorted 派生（而非直接迭代 set）——在遍历顺序上不依赖
        #   PYTHONHASHSEED。现行判据（in / ==）本就与顺序无关，此处加固以防
        #   未来有人在循环里加"取第一个角色"之类的顺序敏感逻辑。
        roles = sorted({self.neurons[m].role for m in member_ids if m in self.neurons})
        if "motor" in roles:
            return "motor"
        if roles == ["sensory"]:
            return "sensory"
    ...
    def _phase_bind(self, active: List[int], l0_enabled: bool = True,
        ...
        # ★v3.4 清理：np.lexsort 要求所有 key **形状完全相同**（无轴广播），
        #   故次键用 np.broadcast_to 生成只读视图（0 分配），替代原 .repeat().repeat() 实体分配
        neg_idx = np.broadcast_to(-np.arange(S.shape[2]), S.shape)     # Na × H × T 只读视图
        order = np.lexsort((neg_idx, -S), axis=2)[:, :, :min(ROUTE_K, S.shape[2])]
        #                                                               # Na × H × K
...
class SensoryPort:
    ...
    ★v3.10（I-1 修复，2026-09-19）—— 入口三层压缩治理
    实测病灶（`diag/diag_entry_budget.py`，基线 std=0.0283）：
      L1 `raw[:dim]` 切片：128→16 维，|v| 1.0000→0.3804（能量只剩 14.47%）
      L3 静态 tanh：峰值仅 0.1936，与下游 PLASTIC_THRESHOLD=0.35 数量级错配
      L5 `abs(a)`：符号被抹，兴奋/抑制不可分

    ★关键否定结论（实测，勿再走回头路）：
      (a) 「128→N 的保距投影」**数学上不存在**（秩瓶颈：N<dim 装不下球面）。
          列正交 Q(128×16) 的 |Q^T v| 只有 0.2803/1.0 → 丢 92.14% 能量；
          行正交 Qn(16×128) 的 |Qn v| 只有 0.3544/1.0。两者都**不保距**。
      (b) 「tanh 前加增益」**无法达标**：实测 GAIN=8 触顶 std=0.1189 < 0.14，
          GAIN=16 起反崩（全饱和到 1，std→0.0097）。单调映射天花板 ≈0.119。
      (c) 「缩放对齐 PLASTIC_THRESHOLD」**不可行**：std 是绝对量，
          峰值缩到 0.35 后 std 同比例塌（0.1799→0.0521）。

    采用方案（实测 std 均值 0.1799，6.36× 于基线）：
      1. 无偏收缩 128→N（等权随机基，非切片 → 不再系统性丢弃尾部 112 维）
      2. L2 归一（消尺度漂移，只留方向/模式）
      3. 对比度增强 `softmax(|u|/T)`，T=0.5
         —— 这是唯一同时满足「std≥0.14」与「区分度 cos<0.95」的非单调映射；
            T=0.2 虽 std 更高(0.2799)但 cos 到 0.9564（输入之间太像），不选。
      4. 保符号 `sign(u)`（L5 修复）
      5. 能量覆盖 top-k（L4 修复）

    ★回退开关：环境变量 `BIO_INPUT_V2=0` → 逐位回退 V1 行为（切片+tanh+abs）。
    """

    SPARSITY = 0.5          # 点亮比例（前 50% 最强的）
    CONTRAST_T = 0.45       # ★v3.10 对比度温度：softmax(|u|/T)
    #   T 扫描实测（diag/diag_i1_tune.py）：
    #     T=0.50 → std 0.1684 但**单条最低仅 0.1326 < 0.14**（靠均值勉强过）
    #     T=0.45 → std 0.1808 且**单条最低 0.1438 ≥ 0.14**（每条独立达标），cos 0.4567
    #     T=0.30 → std 0.2306 更高，但余量已够且 T 越低区分度越差 → 取 0.45
    ...
    def __init__(self, modality: str, neuron_ids: List[int], dim: int):
        ...
        self.dim = dim
        # 固定随机投影（保证同一模态稳定）
        # ★v3.4 修复 PYTHONHASHSEED 暗雷：原用 abs(hash(modality)) 播种，
        #   而 str.__hash__ 受 PYTHONHASHSEED 影响 → 同一份代码在不同进程里
        #   proj 不同，污染"跨进程保持"与"跑两次分数相同"两条铁律。
        #   改用 hashlib.md5 的确定性整数种子（跨进程/跨平台稳定）。
        rng = np.random.default_rng(_det_seed(modality))
        self.proj = rng.normal(0, 1.0 / np.sqrt(max(dim, 1)), (dim, len(neuron_ids)))
        # ★v3.10：无偏收缩基 W (N, dim)。用独立种子，避免与 proj 共用随机流
        #   （共用会改变 V1 的 proj → 破坏 V1 逐位回退）。
        rng2 = np.random.default_rng(_det_seed(modality, "i1_align_v2"))
        self._W = rng2.normal(size=(len(neuron_ids), dim)) / np.sqrt(max(dim, 1))

    # ---------- ★v3.10 I-1 内部工具 ----------
    @staticmethod
    def _v2_enabled() -> bool:
        """回退开关：BIO_INPUT_V2=0 → V1。每次读取，便于测试内切换。"""
        return os.environ.get("BIO_INPUT_V2", "1") != "0"

    def _align(self, raw: np.ndarray) -> np.ndarray:
        """L1 修复：无偏收缩 128→N（替代 `raw[:dim]` 切片）+ L2 归一。

        ★不是"保距投影"（见类 docstring (a)：dim>N 时保距无解）。
        目的是消除切片造成的「尾部 112 维整体丢失」偏置，
        并把尺度归一，使后续对比度增强只作用于**模式**而非幅度。
        """
        r = np.asarray(raw, dtype=float).flatten()
        if r.shape[0] < self.dim:
            r = np.pad(r, (0, self.dim - r.shape[0]))
        elif r.shape[0] > self.dim:
            r = r[:self.dim]                       # 仅在 dim 已是目标维时截齐
        u = self._W @ r
        nrm = float(np.linalg.norm(u))
        return u / nrm if nrm > 1e-12 else u
        ...
        if not self._v2_enabled():
            ...
            for idx in order:
                ...
                if abs(a) > ACTIVE_EPS:
                    ...
                    ignited[nid] = abs(a)
            return {"modality": self.modality, "ignited": ignited, "_mode": "v1"}

        # ---- V2 路径（I-1 修复）----
        # 1) L1+L2：无偏收缩 + 归一（不再切片丢 86% 能量）
        u = self._align(raw)
        # 2) L3：对比度增强（替代静态 tanh —— tanh 单调，天花板 0.119 不达标）
        z = np.abs(u) / max(self.CONTRAST_T, 1e-9)
        z = z - z.max()                            # 数值稳定
        # 3) L5：保符号（替代 abs —— 兴奋/抑制必须可分）
        acts = np.sign(u) * np.exp(z)
        # 4) L4：能量覆盖稀疏 —— top-k，且把被丢弃能量按比例补回保留项
        k = max(1, int(round(len(acts) * self.SPARSITY)))
        order = np.argsort(-np.abs(acts))[:k]
        ignited = {}
        for idx in order:
            ...
            if abs(a) > ACTIVE_EPS:
                nid = self.neuron_ids[idx]
                ns.queue_ignition(nid, a)          # ★带符号
                ignited[nid] = a
        return {"modality": self.modality, "ignited": ignited, "_mode": "v2"}
